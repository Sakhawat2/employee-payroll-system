 ...
 * @param {Object} company - Company info (name, address, city, businessId, logo)
 * @param {Object} payrollData - Payroll data (period, payday, rate, totalHours, totalPay)
 */
export async function generatePayslip(employee, company, payrollData) {
  ...
  autoTable(doc, {
    ...
    body: [
      ["Basic Pay", totalHours.toFixed(2), `${currency}${rate.toFixed(2)}`, `${currency}${totalPay.toFixed(2)}`],
    ],
    theme: "grid",
    styles: { fontSize: 10 },
  });
  ...
  // === Deductions ===
  // === Deductions (dynamic from frontend) ===
const { tax = 14.5, pension = 7.15, unemployment = 0.59 } =
  payrollData.deductions || {};

const deductions = [
  [`Withholding Tax (${tax}%)`, `${currency}${((totalPay * tax) / 100).toFixed(2)}`],
  [`Pension Contribution (${pension}%)`, `${currency}${((totalPay * pension) / 100).toFixed(2)}`],
  [`Unemployment Insurance (${unemployment}%)`, `${currency}${((totalPay * unemployment) / 100).toFixed(2)}`],
];


  // === Summary ===
  const afterDeductions = doc.lastAutoTable.finalY + 10;
  autoTable(doc, {
    startY: afterDeductions,
